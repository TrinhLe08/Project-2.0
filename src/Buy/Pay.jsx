  ...
  //------------------------------------------------------------------
    ...
    const formik = useFormik({
        ...
        validationSchema: Yup.object().shape({
            ...
            email: Yup.string().email('Email invalidate !').required('Email cannot be blank !'),
    ...
    return (
        ...
        <div className="Pay" >
            ...
            <div className="window">
                <h1 className="h1">{!Vip.vietSub ? 'Thanh Toán An Toàn' : 'Secure checkout'}</h1>
            ...
            <div className="cart">
              ...
              <div className="total">  {!Vip.vietSub ? 'Tổng: ' : 'Total price:'} 
               <span className="span">{Vip.Sum}$</span>
                ...
                <form className="form" onSubmit = {formik.handleSubmit}>
                    ...
                    <input className='inputName' id = 'input' type="text" name='name' value={formik.values.name} onChange={formik.handleChange} />

                     {formik.errors.name && formik.touched.name && (<p className="p">{formik.errors.name}</p>)}

                    <label className="label"  htmlFor="email">Gmail : </label>
                    <input type="text" id = 'input' name='email' value={formik.values.email} onChange={formik.handleChange} />

                    {formik.errors.email && formik.touched.email && (<p className="p">{formik.errors.email}</p>)}

                    <div>
                        <label className="label"  htmlFor="country">Country: </label>
                        <select  className='select' component="select" name="country" id="country" onChange={formik.handleChange}>
                            <option value="">  {!Vip.vietSub ? '--Chọn Quốc Gia--' : '--Choose country--'}</option>
                    ...
                    >   {!Vip.vietSub ? 'ĐẶT HÀNG' : 'PLACE ORDER'}</button>
                </form>
            ...
            <h3 className="h3">  {!Vip.vietSub ? 'KHÁCH HÀNG MUA SẢN PHẨM NÀY CŨNG ĐÃ MUA:' : 'CUSTOMERS WHO BOUGHT THIS ITEM ALSO BOUGHT:'}</h3>
